...
// opts.highlightHour: このインデックス（0〜23）のバーだけ opts.highlightColor で塗る（例: 直近24時間チャートの「最新1時間」を強調）
// opts.highlightColor: highlightHour指定時のバー色（省略時は暖色系の目立つ色）
function buildHourlyRadialChart(items, opts) {
  ...
  const highlightHour = (opts.highlightHour !== undefined && opts.highlightHour !== null) ? Number(opts.highlightHour) : null;
  const highlightColor = opts.highlightColor || '#ff8a5c';
  const highlightStroke = opts.highlightStroke || 'rgba(255,138,92,0.75)';
  ...
  for (let h = 0; h < 24; h++) {
    ...
    const isHighlighted = (highlightHour !== null && h === highlightHour);
    const fillColor = isHighlighted ? highlightColor : barColor;
    const strokeColorForBar = isHighlighted ? highlightStroke : barStroke;
    const opacity = isHighlighted ? '0.95' : (0.32 + (val / maxVal) * 0.68).toFixed(2);

    let tooltip = `${pad2(h)}:00 — ${val}件`;
    if (it.avg !== undefined) {
      tooltip += ` / 日平均 ${it.avg}件 / 最大 ${it.max}件 / 最小 ${it.min}件`;
    }
    if (isHighlighted) tooltip += '（進行中の時間帯）';

    barsHtml += `<path class="statsRadialBar" data-tooltip="${escapeAttr(tooltip)}" d="M${x1.toFixed(1)},${y1.toFixed(1)} L${x2.toFixed(1)},${y2.toFixed(1)} A${outerR.toFixed(1)},${outerR.toFixed(1)} 0 ${largeArc} 1 ${x3.toFixed(1)},${y3.toFixed(1)} L${x4.toFixed(1)},${y4.toFixed(1)} A${innerR.toFixed(1)},${innerR.toFixed(1)} 0 ${largeArc} 0 ${x1.toFixed(1)},${y1.toFixed(1)} Z" fill="${fillColor}" fill-opacity="${opacity}" stroke="${strokeColorForBar}" stroke-width="${isHighlighted ? 1.5 : 0.5}" style="cursor:pointer;"></path>`;
    ...
    if (val > 0) {
      ...
      valueLabelsHtml += `<text x="${vx.toFixed(1)}" y="${vy.toFixed(1)}" font-size="8.5" fill="${isHighlighted ? highlightColor : barColor}" font-weight="bold" text-anchor="middle" dominant-baseline="central">${val}</text>`;
    }
    ...
    contentEl.innerHTML = `
      ...
      <div class="statsSection">
        ...
        <div class="statsRadialRow">
          ...
          <div class="statsRadialCol">
            <h4>直近24時間のアクセス数</h4>
            <div class="statsRadialNote">期間の絞り込みによらず常に直近24時間を表示します（オレンジ色のバーは進行中の時間帯）</div>
            <div class="statsRadialSvgWrap">${buildHourlyRadialChart(recentHourly, { size: 400, color: '#5fb3e8', strokeColor: 'rgba(95,179,232,0.6)', gridColor: 'rgba(95,179,232,0.18)', highlightHour: new Date().getHours() })}</div>
          </div>
        </div>
      </div>
